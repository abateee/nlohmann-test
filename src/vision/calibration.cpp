...
CalibrationData CalibrationStore::load(const std::filesystem::path& path)
{
    ...
    {
        throw std::runtime_error("Calibration introuvable: " + path.string());
    }
    ...
    if (path.extension() == ".json" && path.filename().string().find(".fs.") == std::string::npos)
    {
        ...
        if (!input)
        {
            throw std::runtime_error("Impossible d'ouvrir la calibration JSON: " + path.string());
        }
    ...
    if (!storage.isOpened())
    {
        throw std::runtime_error("Impossible d'ouvrir la calibration FileStorage: " + path.string());
    }
...
BoardPoint project_image_point(const CalibrationData& calibration, const cv::Point2d& image_point)
{
    ...
    return BoardPoint{output_points.front().x, output_points.front().y};
}
...
cv::Point2d project_board_point_to_image(const CalibrationData& calibration, const BoardPoint& board_point)
{
    ...
    std::vector<cv::Point2d> board_points = {cv::Point2d(board_point.x, board_point.y)};
